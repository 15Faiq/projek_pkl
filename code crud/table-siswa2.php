<?php
include 'config.php';
$columns = array('nama_atlet', 'tgl_reg', 'ttl', 'jenis_kelamin', 'bb', 'tb', 'no_hp', 'tingkat', 'kelas', 'ket', `ketw`);

$query = "SELECT * FROM data_siswa2 WHERE ";
...
while ($row = mysqli_fetch_array($result)) {
  ...
  $sub_array[] = $row["alamat_wali"];
  $sub_array[] = $row["tgl_reg"];
  $sub_array[] = $row["bb"];
  $sub_array[] = $row["tb"];
  $sub_array[] = $row["no_hp"];
  $sub_array[] = $row["ttl"];
  $sub_array[] = $row["no_hpwali"];
  $sub_array[] = $row["nama_wali"];
  $sub_array[] = $row["nama_atlet"];
  $sub_array[] = $row["jenis_kelamin"];
  $sub_array[] = $row["ket"];
  $sub_array[] = $row["ketw"];
  $sub_array[] = '<center><a href="editsiswa2.php?id=' . $row['no_id'] . '" class="btn btn-info btn-sm"><i class="fas fa-user-edit"></i></a>
 <a href="bayarspp.php?id=' . $row['no_id'] . '" 
 </center>';
  $data[] = $sub_array;
}
