...
include 'config.php';
if (isset($_GET['id'])) {
    $sql = mysqli_query($connect, "SELECT * FROM data_siswa2 WHERE no_id = '$_GET[id]' ");
...
if (isset($_POST['update'])) {
    if (empty($_POST['nama_atlet'])) {
        ...
        $nama_file = $_FILES['foto_siswa2']['name'];
        $tmp_file = $_FILES['foto_siswa2']['tmp_name'];
        $path = "foto-siswa2/" . $nama_file;
        if ($nama_file == '') {
            ...
            <?php
                        ...
                        if (move_uploaded_file($tmp_file, $path)) {
                            $uppdate = mysqli_query($connect, "UPDATE data_siswa2 SET 
                              nama_atlet = '$_POST[nama_atlet]',
                              tgl_reg = '$_POST[tgl_reg]',
                              ttl = '$_POST[ttl]',
                              jenis_kelamin = '$_POST[jenis_kelamin]',
                              ...
                              no_hpwali = '$_POST[no_hpwali]',
                              alamat_wali = '$_POST[alamat_wali]',
                              foto_siswa2 ='" . $nama_file . "' 
                                WHERE no_id = '$_GET[id]' ");
                        }
                        unlink("foto-siswa2/" . $foto_siswa2);
                    }
                    if ($uppdate) {
                        ?>
            <script>
